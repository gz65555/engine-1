...
The render order of the renderer affects the **performance** and **accuracy** of rendering. In Oasishub, for each camera, components are placed in the corresponding **render queue** according to a unified **determination rule**.

## Render Queue

Oasishub is divided into three render queues, in the order of rendering:
...
The determination rules for render order in Oasishub are as follows:
...
<Image src="https://mdn.alipayobjects.com/huamei_w6ifet/afts/img/A*5xikSqZn2tMAAAAAAAAAAAAADjCHAQ/original" alt="Distance to Camera Diagram" style={{ zoom: "50%" }} />
