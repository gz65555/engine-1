...
Oasishub 引擎目前有 3 种方式调试材质：

1. 通过代码修改材质属性，参考[教程](/docs/graphics/material/material)。

2. 通过 Oasishub Editor 可视化调试，参考[教程](/docs/graphics/material/material)。
...
目前在实时渲染领域应用的最多的是 PBR 算法，拥有能量守恒、物理正确、易操作等优点，但是不同软件的具体实现算法是不一样的，使得渲染结果也不一样。Oasishub 使用的是 **Cook-Torrance BRDF** 反射率方程，并针对移动端做了优化。
...
跟现实世界一样，3D 场景也可以添加[直接光与环境光](/docs/graphics/light/light/)。Oasishub 场景中默认是**没有**光源的，只有一个偏向蓝色的[纯色漫反射](/apis/core/#AmbientLight-diffuseSolidColor)，如下图左一；而很多建模软件中是自带光源的：
...
环境光基于 [立方纹理](/docs/graphics/texture/cube) 开启 IBL 模式，需要绑定一张 HDRI 贴图用来模拟周边环境，可以从[网上下载](https://polyhaven.com/hdris)。Oasishub 场景中默认是没有绑定 HDRI 贴图的，而很多建模软件是自带了一张比较好看的周边环境的：
...
前面说到，引擎默认不带直接光，那么保持还原度最简单的方法，就是删除建模软件中的灯光，保证建模软件和 Oasishub 引擎中都只有环境光（性能最好）。
...
前面说到 Oasishub PBR 使用的是 **Cook-Torrance BRDF** 反射率方程，在 Blender 中比较接近的是 Principled BSDF - GGX 算法：
...
我们后期还会投入插件开发人员，在各种建模软件中内置 Oasishub 预览插件，保证所见即所得，省去 glTF 文件校验等步骤。
