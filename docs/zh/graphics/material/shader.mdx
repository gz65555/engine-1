...
`ShaderLab` 是 Oasishub 针对 Shader 自主研发的一套描述语言，它提供了一套完整的语法来定义着色器的结构、属性、渲染状态和着色器代码。通过 ShaderLab，开发者可以创建从简单到复杂的各种视觉效果。
...
渲染状态变量和引擎枚举类型保持一致，比如 `BlendOperation.Add` 对应引擎的 [API](/apis/oasishub/#BlendOperation)。
