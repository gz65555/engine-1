...
Models typically refer to 3D models created by designers using 3D modeling software, containing a series of [meshes](/en/docs/graphics/mesh/mesh/), [materials](/en/docs/graphics/material/material/), [textures](/en/docs/graphics/texture/texture/), and [animations](/en/docs/animation/overview/). In Oasishub, they are also considered assets. The model asset workflow is usually as follows:

```mermaid
	flowchart LR
	A[Modeling software exports model] --> B[Import model into Oasishub editor] --> C[Adjust model]
```
