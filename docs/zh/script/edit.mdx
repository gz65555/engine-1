...
Oasishub Editor 提供了一个功能强大的代码编辑器，提供了代码提示，第三方包引入，引擎事件调试，脚本参数调试，项目实时预览等多种能力，可帮助你快速编辑和调试代码。
...
在场景编辑器中创建脚本资产后，双击该脚本即可打开代码编辑器。Oasishub 中的脚本需使用 [Typescript](https://www.typescriptlang.org/) 语言编写，同时新脚本默认基于内置模板创建。另外，Oasishub 的代码编辑器基于 Monaco，快捷键与 VSCode 类似。修改脚本后，按 `Ctrl/⌘ + S` 保存，右侧实时预览区展现最新场景效果。

> 提示：Oasishub 代码编辑器目前支持 `.ts` `.gs` 和 `.glsl` 的文件编辑
...
代码编辑器内置了与项目相对应的引擎，可自动提供引擎 API 的智能提示，从而帮助你快速实现基于引擎的逻辑。但大多数情况下你都需要引入 Oasishub 生态包或其他第三方包来增强功能。
...
<Callout type="info">
试一下：在搜索框输入 `@oasishub/engine-toolkit`，点击「引入」按钮，然后在代码中使用 `import { OrbitControl } from '@oasishub/engine-toolkit` 来引入 `OrbitControl` 组件。
</Callout>


## 实时预览区

Oasishub 的代码编辑器提供实时预览功能。保存代码后，预览区会自动更新，从而让你快速查看代码的执行结果。
...
1. **事件列表** Oasishub Editor 会自动收集场景中的所有事件名并显示在这里
