...
<Comparison
  ...
  rightText="FXAA ON"
/>
Oasishub 中默认 `FXAA` 关闭，你可以通过相机下的`抗锯齿`进行配置，或者直接调用 [antiAliasing](/apis/core/#Camera-antiAliasing) 接口设置。
...
Oasishub 中 `MSAA采样` 默认为`4倍`，你可以修改相机下 `MSAA采样` 来自定义 `MSAA` 质量，或者直接调用 [msaaSamples](/apis/core/#Camera-msaaSamples) 接口设置。
