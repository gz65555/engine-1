...
- **(.glb)**：适合模型文件较大的场景，会将所有数据进行二进制保存，需要等所有数据解析完毕才能展示模型，Oasishub 对这两种产物

以上两种产物在 Oasishub 中都已支持，如何选择产物的导出类型可以按照项目实际情况决定。

## Oasishub 对 glTF 的支持

**glTF2.0** 是目前 Oasishub 推荐的首选 3D 场景传输格式，Oasishub 对 **glTF2.0** 的核心功能和插件都做了很好的支持：
...
glTF 拥有非常多的特性，官网提供了大量的[示例](https://github.com/KhronosGroup/glTF-Sample-Models/tree/master/2.0)进行参考。

### 插件支持

Oasishub 目前支持以下 glTF 插件，若 glTF 文件中包含相应插件，则会自动加载相应功能：
...
| [KHR_materials_unlit](https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_materials_unlit) | 支持 [Unlit 材质](/docs/graphics/material/builtinShaders/unlit/) |
