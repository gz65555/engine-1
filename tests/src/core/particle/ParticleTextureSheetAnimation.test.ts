...
describe("TextureSheetAnimation Test", () => {
  ...
  it("Clone", () => {
    ...
    const cloneTextureSheetAnimation = renderer.entity.clone().getComponent(ParticleRenderer)
      .generator.textureSheetAnimation;
    expect(cloneTextureSheetAnimation.tiling).to.deep.include({ x: 4, y: 4 });
    // @ts-ignore
    expect(cloneTextureSheetAnimation.tiling._onValueChanged).to.not.equal(
      textureSheetAnimation.tiling._onValueChanged
    );
    // @ts-ignore
