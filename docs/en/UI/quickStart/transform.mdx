...
`UITransform` is a transform component specifically designed for UI elements in the Oasishub engine, inheriting from the base `[Transform](/apis/core/#Transform)` component. It provides rich layout and alignment features and is a core component for building flexible UI systems.
...
```typescript
import { UITransform, HorizontalAlignmentMode, VerticalAlignmentMode } from "@oasishub/engine";
...
The UITransform component provides powerful and flexible UI layout capabilities for the Oasishub engine. By properly using these features, you can build beautiful UI interfaces that adapt to different screen sizes.