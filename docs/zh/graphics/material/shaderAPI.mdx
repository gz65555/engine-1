...
提供了`PI` 等常用宏，`gammaToLinear`、`pow2` 等通用方法，详见源码。
...
封装了基于 BSDF 光照模型的[环境光](/docs/graphics/light/ambient)计算，详见源码。
...
PBR 光照模型关键文件，封装了 BRDF、投射、折射等相关的通用计算函数， 以及用于后续光照模型计算的 `SurfaceData` 结构体和 `BSDFData` 结构体，详见源码。
