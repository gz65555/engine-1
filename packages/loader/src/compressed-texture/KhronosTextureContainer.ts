...
import { TextureFormat } from "@oasishub/engine-core";
import { GLCompressedTextureInternalFormat } from "@oasishub/engine-rhi-webgl";
...
function getEngineFormat(internalFormat: GLint): TextureFormat {
  switch (internalFormat) {
    ...
    default:
      ...
      throw new Error(`this format is not supported in Oasishub Engine: ${formatName}`);
  }
}
/**
...
export const khronosTextureContainerParser = {
   ...
   * @param mapEngineFormat get Oasishub Engine native TextureFormat?
   */
  parse(
    buffer: ArrayBuffer,
