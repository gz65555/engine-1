...
Oasishub Editor provides a powerful code editor with features such as code suggestions, third-party package imports, engine event debugging, script parameter debugging, real-time project preview, and more, helping you quickly edit and debug code.
...
After creating a script asset in the scene editor, double-click the script to open the code editor. Scripts in Oasishub need to be written in [Typescript](https://www.typescriptlang.org/), and new scripts are created based on built-in templates by default. Additionally, the Oasishub code editor is based on Monaco, with shortcuts similar to VSCode. After modifying the script, press `Ctrl/⌘ + S` to save, and the real-time preview area on the right will show the latest scene effects.

> Tip: The Oasishub code editor currently supports `.ts`, `.gs`, and `.glsl` file editing.
...
<Callout type="info">
Try it: Enter `@oasishub/engine-toolkit` in the search box, click the "Import" button, and then use `import { OrbitControl } from '@oasishub/engine-toolkit` in the code to import the `OrbitControl` component.
</Callout>

## Real-time Preview Area

The Oasishub code editor provides a real-time preview feature. After saving the code, the preview area will automatically update, allowing you to quickly see the execution results of the code.
...
1. **Event List** The Oasishub Editor will automatically collect all event names in the scene and display them here.
