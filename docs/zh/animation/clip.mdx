...
`动画片段`（[API](/apis/core/#AnimationClip)）是 Oasishub 动画系统的核心元素之一，Oasishub 支持导入外部设计软件设计的模型动画，设计师输出的带动画的模型中的每个动画在 Oasishub 中会被解析成一个个的 `动画片段` 资产，我们也可以通过动画片段编辑器创作额外的动画。
...
2. 在 Oasishub 中创建 `动画片段`（下文会介绍编辑器和脚本这两种创建方式）
...
- **文字动画**：除了 `Transform` 组件，Oasishub 还支持编辑其他组件的动画，可以阅读[文字动画](/docs/animation/examples/text-animation)文档来了解。
- **帧动画** ：除了数值类型，Oasishub 还支持引用类型的 `动画曲线`， 可以阅读[帧动画](/docs/animation/examples/sprite-sheet)文档来了解。
- **材质动画** ：Oasishub 还支持组件中的资产属性的动画编辑，可以阅读[材质动画](/docs/animation/examples/material-animation)文档来了解。
