...
ShaderLab is a proprietary shader description language developed by Oasishub. It provides a complete syntax for defining the structure, properties, rendering states, and shader code of a shader. With ShaderLab, developers can create a wide range of visual effects, from simple to complex.
...
The rendering state variables are consistent with the engine's enumeration types. For example, `BlendOperation.Add` corresponds to the engine's [API](/apis/oasishub/#BlendOperation). #### 1.1 BlendState - Blending State
...
import { registerIncludes } from "@oasishub/engine-shader";
...
```ts
import { ShaderFactory } from '@oasishub/engine';
...
```ts showLineNumbers {}
import { ShaderUIScript, Material } from "@oasishub/engine";
