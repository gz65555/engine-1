...
`AnimationClip` ([API](/apis/core/#AnimationClip)) is one of the core elements of the Oasishub animation system. Oasishub supports importing model animations designed with external design software. Each animation in the model output by the designer will be parsed into an `AnimationClip` asset in Oasishub. We can also create additional animations through the animation clip editor.
...
2. Create `AnimationClip` in Oasishub (the following will introduce both editor and script creation methods).
...
- **Text Animation**: In addition to the `Transform` component, Oasishub also supports editing animations of other components. You can read the [Text Animation](/en/docs/animation/examples/text-animation) document to learn more.
- **Frame Animation**: In addition to numerical types, Oasishub also supports reference type `animation curves`. You can read the [Frame Animation](/en/docs/animation/examples/sprite-sheet) document to learn more.
- **Material Animation**: Oasishub also supports animation editing of asset properties in components. You can read the [Material Animation](/en/docs/animation/examples/material-animation) document to learn more.
