...
用户可以在 Oasishub 中轻松完成 Lottie 资产的处理和组件添加。
...
开发者拿到 `.json` 文件后，首先需要把 `.json` 文件上传到 Oasishub Editor。通过资产面板的上传按钮选择 “lottie” 资产，选择本地一个 lottie json 文件，然后上传：
...
@oasishub/engine-lottie 是 Oasishub Engine 的二方包，项目中用到了 Lottie 的时候，需要确保项目中安装了该包：
...
在进行 `Pro Code` 开发的时候，需要一个 `json` 文件和一个 `atlas` 文件来实现 `lottie` 动画，通常美术同学通过 `AE` 导出的给到开发的只有 `json` 文件，此时需要使用 tools-atlas-lottie `CLI` 工具生成 `atlas` 文件。
